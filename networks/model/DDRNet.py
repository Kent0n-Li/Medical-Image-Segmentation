...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_ch, num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=False):
        ...
        self.conv1 =  nn.Sequential(
                          nn.Conv2d(in_ch,planes,kernel_size=3, stride=2, padding=1),
...
def DDRNet(in_ch, out_ch):

    num_classes = out_ch
    model = DualResNet(BasicBlock, [3, 4, 6, 3], in_ch = in_ch, num_classes=num_classes, planes=64, spp_planes=128, head_planes=256, augment=False)
    return model
